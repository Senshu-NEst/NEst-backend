...
def generate_random_date():
    ...
    # ISO 8601形式でフォーマット
    return random_date.astimezone(pytz.timezone("Asia/Tokyo")).isoformat()

# ランダムにリクエストを生成する関数
def generate_random_sale_request():
    store_code = random.choice(config["store_codes"])
    sale_products = []
    
    # ランダムに商品を生成
    for _ in range(random.randint(3, 10)):  # 2〜5個の商品のリスト
        jan = random.choice(config["jans"])
        quantity = random.randint(1, 10)  # 1〜10の範囲で数量を設定
        discount = round(random.randint(0, 50),-1)
    ...
    # 支払い方法をランダムに生成
    payments = []
    cash_amount = random.randint(*config["payment_limits"]["cash"])
    credit_amount = round(random.randint(*config["payment_limits"]["credit"]),-1)
    ...
    terminal_id = store_code + "-P" + str(random.randint(1000,9999))
    # リクエストの構造体を生成
    sale_request = {
        "status": "sale",
...
for _ in range(config["request_count"]):  # 指定した回数リクエストを送信
    request_data = generate_random_sale_request()
    request_json = json.dumps(request_data, ensure_ascii=False)

    try:
        response = requests.post('http://nest-store.f5.si:8500/api/transactions/', data=request_json, headers={'Content-Type': 'application/json'})
        #print(response.text)
        # ステータスコードをチェック
        if response.status_code == 201:  # 成功した場合
            success_count += 1
        else:  # それ以外は失敗
            failure_count += 1
    except Exception:
