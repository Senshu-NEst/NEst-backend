...
def generate_receipt_text(transaction):
    ...
    if transaction.status == "return": 
        status = "************返品済取引*************"
    elif transaction.status == "training":
        status = "***********トレーニング***********"
    ...
    if transaction.status == "resale":
        resale = "*"
        canceled_id = f'{transaction.store_code.store_code}-{transaction.relation_return_id.origin_transaction.id}'

    receipt = f"""{{image:{image}}}
| {transaction.store_code.name} |
| {status} |
|{timezone.localtime(transaction.date).strftime('%Y年%m月%d日 %H:%M')}
...
"""
    ...
    for product in sale_products:
        jan_last_3_digits = product.jan[-3:]
...
"""
    ...
    if wallet:
        ...
        if previous_wallet_transaction:
            ...
            used_amount = 0  # 今回利用金額を取得
            post_balance = previous_wallet_transaction.balance  # 利用後残高を取得
            if current_wallet_transaction:
                used_amount = current_wallet_transaction.amount
                post_balance = current_wallet_transaction.balance
    ...
    if canceled_id:
        receipt += f"""{{width:*}}
            |取消取引:{canceled_id}\n"""
    receipt += f"\n{{code:{sale_id}; option:code128,3,48,nohri}}\n"
...
def generate_return_receipt(returntransaction):
    
    return_type = returntransaction.return_type
    return_id = returntransaction.id
    origin_transaction = returntransaction.origin_transaction
    modify_id = returntransaction.modify_id
    return_date = returntransaction.return_date
    terminal_id = returntransaction.terminal_id
    store_code = returntransaction.store_code
    staff_code = returntransaction.staff_code.staff_code
    staff_name = returntransaction.staff_code.name
    return_products = returntransaction.return_details.all()
    return_payments = returntransaction.return_payments.all()

    receipt = f"""{{image:{image}}}
| {store_code.name} |
|{return_date.strftime('%Y年%m月%d日 %H:%M')}
|担当:{staff_code}{staff_name}
|取引ID:{terminal_id}-{return_id}
{{border:none}}
{{border:line; width:22}}
^返品明細
{{border:space; width:3,*,8; text:nowrap}}
    ...
    for product in return_products:
        ...
        if product.quantity == 1:
            if product.discount > 0:
                # 1点のみ、値引きあり → 2行
                receipt += f"{jan_last_3_digits} |{product.name} | ¥{-product.price:,}{is_reduced_tax}\n"
                receipt += f"  ||~~~~~Δ値引 @{product.discount:,} | ¥{product.discount:,}{is_reduced_tax}\n"
            else:
                # 1点のみ、値引きなし → 1行
                receipt += f"{jan_last_3_digits} |{product.name} | ¥{-product.price:,}{is_reduced_tax}\n"
        else:
            ...
            if product.discount > 0:
                ...
                receipt += f"  ||~~~~~@{-product.price:,}*{product.quantity} | ¥{-product_total_price}{is_reduced_tax}\n"
                total_discount = product.discount * product.quantity  # 合計値引き額
                receipt += f"  ||~~~~~Δ値引 @{product.discount:,}*{product.quantity} | ¥{total_discount:,}{is_reduced_tax}\n"
            else:
                # 複数点、値引きなし → 2行
                receipt += f"{jan_last_3_digits} |{product.name}\n"
                receipt += f"  ||~~~~~@{-product.price:,}*{product.quantity} | ¥{-product_total_price}{is_reduced_tax}\n"
...
"""
    ...
    receipt += f"|元 取 引：{origin_transaction.id}\n"
    if modify_id:
        receipt += f"|再売取引：{modify_id.id}\n"

    # バーコードの挿入
    receipt += f"{{code:{return_id}; option:code128,3,48,nohri}}\n"
